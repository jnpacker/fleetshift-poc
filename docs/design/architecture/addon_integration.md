...
- **Connect** (Enabled → Connected): the addon workload provides its runtime assets — delivery agents, extension resource schemas (inline proto sources; management and/or inventory), and target definitions. The platform compiles schemas, registers dynamic gRPC/HTTP services for each schema's declared capabilities (including inventory-only dual registration), wires delivery agents into the router, and seeds targets. Connect is idempotent on reconnection: schemas that haven't changed are left in place (content-hashed by the activator), stale schemas are deactivated, and new schemas are compiled and registered. Connect is irrelevant for addons that only declare capabilities without runtime workload assets (e.g. UI-only addons).
- **Disconnect** (Connected → Enabled): the delivery agent is deregistered, but the API surface (gRPC/HTTP for extension resources) stays live so consumers can still read resources and, for management-capable types, submit CRUD. This reflects the design intent that addon unavailability should not prevent users from submitting or reading resources — delivery is paused, not the API.
...
- **`ManagedResourceCapability`**: declares that the addon will provide a management-capable extension resource type (e.g. `clusters`). Requires Connect. The concrete schema arrives at connect time as an `ExtensionResourceSchema` with a `Management` section (inline proto files, singular/plural, proto package, fulfillment relation). Management-capable types expose Create/Delete/Resume and writable extension `labels`.
- **`InventoryResourceCapability`**: declares that the addon will provide inventory reporting for an extension resource type. Requires Connect. The schema's `Inventory` section activates inlined observed-state fields on Get/List (`local_labels`, `conditions`, `observation`, timestamps). Inventory-only types activate a read-only extension surface. Management and inventory capabilities compose on the same type when both are declared.
- Activation always implies dual registration: both the extension resource type (in the addon's own package) and the corresponding platform resource type (for canonical identity), including for inventory-only schemas. The platform validates at connect time that every schema section matches a declared capability.
...
An addon can declare multiple capabilities of different types. For example, a cluster management addon might declare a `ManagedResourceCapability` and `InventoryResourceCapability` for `clusters`, a `DeliveryCapability` for its provisioning target type, and a UI plugin capability for its management dashboard — all in a single addon descriptor.
...
Addons extend the platform's gRPC and HTTP API surface at runtime. Each extension resource schema activation — whether management-capable, inventory-only, or both — performs **dual registration** in the two-layer API model (see [resource_identity_and_api.md](resource_identity_and_api.md)):

1. **Extension service**: registered under the addon's own proto package (e.g. `kind.fleetshift.v1.ClusterService`), providing the typed API for the type's declared capabilities (management fields and/or inlined observed-state fields). HTTP routes are at `/apis/{service_name}/{version}/{resource_path}`, where `resource_path` uses the shared collection identifier for that identity domain (for example `clusters/{id}`).
2. **Platform service**: registered under the platform package for the corresponding platform resource type, providing the generic canonical identity surface (labels, effective_labels, conditions, representations, aliases). HTTP routes are at `/apis/fleetshift.io/{version}/{resource_path}` for the same collection identifier. Inventory-only schemas activate this platform surface as well; they establish platform identity even without management methods.
...
- **`extensionresource`** (extension + activator): service builder and gRPC/HTTP handlers for addon-defined extension APIs, plus the `DynamicSchemaActivator` that orchestrates schema compilation and registration.
...
`dynamicapi` is a pure leaf — both `extensionresource` and `platformresource` import it, and `extensionresource` imports `platformresource` (the activator registers platform services as a side-effect of extension activation). There are no cycles.
