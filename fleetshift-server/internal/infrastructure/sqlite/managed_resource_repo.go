...
func (r *ManagedResourceRepo) scanInstance(s interface{ Scan(...any) error }) (domain.ManagedResource, error) {
...
func (r *ManagedResourceRepo) scanView(s interface{ Scan(...any) error }) (domain.ManagedResourceView, error) {
	...
	// Fulfillment scan vars (matching scanFulfillment order)
	...
	if err := s.Scan(
		...
		// resource_intents columns
		&riSpec, &riCreatedAt,
		// fulfillment columns (from fulfillmentColumnsJoined)
	...
	// Hydrate fulfillment (same logic as scanFulfillment)
