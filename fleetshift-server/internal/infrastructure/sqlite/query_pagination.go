...
type querySupportedOrder struct {
	...
	// on the final SELECT after page-window hydration.
...
// compatible. Keep in lockstep with the Postgres query page token
// version so opaque tokens remain interchangeable across backends
// for the same filter/order_by/cursor payload.
